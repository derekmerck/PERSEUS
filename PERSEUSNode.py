...
class ControlNode(PyroNode):
    ...
    def setup_rules(self, config):
        self.rules = ParseRules(config)

    def test_rules(self, values, source=None):
        alert = TestRules(self.rules, values, source)
        if alert:
            # Need to keep track that this alert has been dealt with within x ms
            self.logger.debug(alert.msg)
...
#        self.logger.debug(node_values)
        ...
        for node, values in node_values.iteritems():
            if self.test_rules(values, node):
